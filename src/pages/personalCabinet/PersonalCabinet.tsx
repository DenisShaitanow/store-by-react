...
const PersonalCabinet: FC = () => {
    ...
    const handleChangeAvatar = (val: File) => {
        setPersonalCabinetData((prev) => ({...prev, avatar: URL.createObjectURL(val)}));
    }
    ...
    const handleAvatar = (newImage: File) => {
        setPersonalCabinetData((prev) => ({...prev, avatar: URL.createObjectURL(newImage)}));
    }
    ...
    return (<div className={styles.container}>
        ...
        <FormUserInformationStepTwo hiddenAvatarInput genderOptions={[{value: 'man', label: 'Мужской'}, {value: 'woman', label: 'Женский'}]}
        ...
        />
        <ButtonUI label='Изменить данные' className={styles.changeData} onClick={handleUpdatePersonalInformation}/>
    </div>
    )
}
